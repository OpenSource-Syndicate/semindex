...
class Indexer:
    ...
    def _process_file(self, language_name: str, adapter, path: str) -> Tuple[str, List[Symbol], List[Chunk], str, List[Tuple[str, str]], bool]:
        """Process a single file for parallel execution"""
        try:
            from .crawler import read_text
            from .store import file_sha256_from_content
            ...
            try:
                ...
                result = _parse_and_chunk(language_name, adapter, path, source, self.embedder, self.chunk_config)
                calls_local = result.calls or []
            except Exception as exc:
                parse_failed = True
            ...
            if parse_failed:
                from .cli import _make_module_symbol
            ...
            if not chunks:
                from .cli import _default_chunks_for_symbols
                chunks = _default_chunks_for_symbols(source, symbols)

            return path, symbols, chunks, current_hash, calls_local, parse_failed
        except Exception as e:
            # Return empty results on failure
            return path, [], [], "", [], True

    def index_path_parallel(
        self,
        repo: str,
        incremental: bool = True,
        *,
        ...
        from .cli import _resolve_targets, _format_symbol_row
        from .crawler import iter_python_files, read_text
        from .store import DB_NAME, FAISS_INDEX, add_vectors, add_calls, ensure_db, db_conn, reset_index, get_changed_files, file_sha256_from_content
        from .model import Chunk
        from .embed import Embedder
        from tqdm import tqdm
        from .cli import _make_module_symbol, _default_chunks_for_symbols
        ...
        if incremental and os.path.exists(db_path) and os.path.exists(index_path):
            changed_files = get_changed_files(repo, db_path, language=language)
            base_targets = _resolve_targets(changed_files, language)
            # _resolve_targets returns [(language_name, adapter, path)]
            targets = [(language_name, adapter, path) for language_name, adapter, path in base_targets]
            if verbose:
                print(f"Found {len(targets)} changed/new files for incremental indexing")
        else:
            from .languages import collect_index_targets
            base_targets = collect_index_targets(repo, language)
            # collect_index_targets returns [(adapter, path)]
            targets = [(adapter.name, adapter, path) for adapter, path in base_targets]
            reset_index(index_dir, dim=self.embedder.model.config.hidden_size)
            if verbose:
                print(f"Performing fresh indexing of {len(targets)} files")
        ...
        if verbose:
            print(f"Processing {len(targets)} files with {max_workers} workers")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in tqdm(concurrent.futures.as_completed(future_to_target), 
                              ...
                              unit="file"):
                target_tuple = future_to_target[future]
                try:
                    result = future.result()
                    if result is not None:
                        path, symbols, chunks, current_hash, calls_local, parse_failed = result
                        ...
                        if parse_failed:
                            ...
                            all_calls.extend(calls_local)
                    else:
                        # Extract path from target tuple for failed processing
                        if len(target_tuple) >= 3:
                            failed_files.append(target_tuple[2])  # path is third element
                        else:
                            failed_files.append(str(target_tuple))
                except Exception as e:
                    # Extract path from target tuple for exception handling
                    if len(target_tuple) >= 3:
                        failed_files.append(target_tuple[2])  # path is third element
                    else:
                        failed_files.append(str(target_tuple))
                    if verbose:
                        print(f"[WARN] Failed to process {target_tuple}: {e}")
        ...
        with db_conn(db_path) as con:
            ...
            if all_calls:
                ...
                call_rows = []
                seen_edges = set()
                for path, caller_name, callee_name in sorted(all_calls):
                    caller_id = symbol_id_map.get((path, caller_name))
                    if not caller_id:
                        continue
                    callee_id = None
